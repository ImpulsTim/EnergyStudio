function drawPiek(allTs,perKw,grpKw,cos){
  ...
  function makePiekChart(canvasId,indArr,somArr,collArr){
    var barDS=cos.map(function(c,i){return{label:c.name,data:indArr[i],backgroundColor:PAL[i%PAL.length]+'55',borderColor:PAL[i%PAL.length],borderWidth:1,borderRadius:3,stack:'ind'};});
  ...
  gtoHtml+='<tr style="background:#edf5ea;font-weight:700"><td>Totaal/gem.</td><td>'+(totSomInd/mnds.length).toFixed(1)+' gem.</td>'+
    '<td>'+(collP.reduce(function(s,v){return s+v;},0)/mnds.length).toFixed(1)+' gem.</td>'+
    '<td style="color:#46962b">gem. '+(totDiv/mnds.length).toFixed(1)+' kW / '+(totSomInd>0?(totDiv/totSomInd*100).toFixed(0):0)+'%</td>'+
...
function updateSim(){
  ...
  CH['simA']=new Chart(document.getElementById('cSimA'),{type:'bar',data:{labels:mLbl,datasets:[
    {label:'Collectieve afnamepiek -'+pct+'% (kW)',data:newPA,backgroundColor:'rgba(192,57,43,.6)',borderRadius:4,stack:'a'},
    {label:'Ruimte binnen GTV afname (kW)',data:spaceA,backgroundColor:'rgba(70,150,43,.5)',borderRadius:4,stack:'a'},
  ]},options:chartOpts(gtvA)});

  CH['simT']=new Chart(document.getElementById('cSimT'),{type:'bar',data:{labels:mLbl,datasets:[
    {label:'Collectieve terugleveringspiek -'+pct+'% (kW)',data:newPT,backgroundColor:'rgba(230,126,34,.6)',borderRadius:4,stack:'a'},
    {label:'Ruimte binnen GTV-T teruglevering (kW)',data:spaceT,backgroundColor:'rgba(70,150,43,.5)',borderRadius:4,stack:'a'},
